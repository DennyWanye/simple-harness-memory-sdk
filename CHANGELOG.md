...
## [0.6.38] - 2026-09-09（租约到期降级为收据；冲突组 incumbent 进向量世代；世代覆盖率与可用性分开判）

- **缺陷来源**：**F-AA-2** = Host 事件 AA 备忘 `simple_harness/plans/2026-09-08-hm-to-a6/DECISION-AA-AUTHORITY-STALE-RECOLLECT.md` §2 / §9(2)（HM-TO-A6 T18）；**F-V-2a / F-V-2b** = 本仓 0.6.37 备忘 `DECISION-2026-09-09-contested-group-admission-basis.md` §5.1 / §8（当时记为 P2，本轮升级为交付项）。裁决与逐条论证见 `plans/2026-08-29-human-memory-digital-twin/DECISION-2026-09-09-lease-degradation-and-incumbent-vectors.md`。
- **F-AA-2 现象**：T18 一轮工具循环 12 次 provider 请求，前 11 次都在租约内正常出收据，第 12 次比 `authority_expires_at` 晚了 **4.96 秒** → `RECALL_AUTHORITY_STALE` → Host `RecallContextUseAuthorityStale` → `run.fail`；那一刻 **7 条被绑定来源逐条未变**。租约的上游是 Host 的 `RecallContext.expires_at = moment + 60.0`——**一个召回上下文期限被当成了整轮用途授权的租约**，只要一轮工具步骤跑过一分钟这一轮必死。
- **F-AA-2 修法**：`effective_now >= result.authority_expires_at` 从用途围栏的硬失败判据里移出，降级为稳定码 `authority_lease_expired`（照常签发收据 + 一行无载荷日志）。理由是结构性的：`authority_expires_at = min(RecallContext.expires_at, 每条被绑定来源自己的期限)`，而后者在同一把写锁、同一事务的逐来源重校验里被**逐条独立重新执行**（认知记忆 `valid_from <= now < valid_to`，Short-Horizon chunk `now < expires_at`）。租约唯一多挡住的就是那个 60 秒的上下文期限，它挡不住任何一条真的失效了的来源。**policy version、本部署策略版本、权威 epoch 倒退仍硬失败；逐来源重校验一字未动**，任一来源真的变了/被抑制/被降级披露仍以同一稳定码、零 payload、零收据行拒绝。
- **续租**：冻结的 `RecallContextUseReceiptV1` 要求 `expires_at > authorized_at`（"已过期的收据"在契约上不可构造），故这一支**续发**一段租约：长度 = 结果原本的租约长度（`authority_expires_at - evaluated_at`），起点 = 本次授权时刻，**上界 = 本次重校验重新读到的来源最早期限**——续租永远不会让模型用到一条已经过期的来源。为此 `_validate_recall_context_use_sources_unlocked` 额外返回该上界（判据一条未改，召回路径忽略返回值）。租约**未**到期那一支收据逐字节不变（`NO_RACE_RECEIPT_HASH` 用例照旧绿）。
- **降级说明仍然零 DDL**：`authority_lease_expired` 由不可变的 `recall_context_use_receipts.authorized_at` 与被绑定结果 `result_json` 的 `authority_expires_at` 严格导出——**不是**收据行自己那个续发的 `expires_at`。`RecallContextUseAuthorityNoteV1` 追加带默认值的可选字段 `bound_authority_expires_at`（既有 13 参位置式构造逐字不变），epoch 码上必须为 `None`；epoch 不变性按码分岔（epoch 码要求严格前进，租约码允许相等但永不倒退）。一张收据可同时导出两条说明，次序固定为 epoch 前进在前。
- **F-V-2a 修法**：认知向量世代的覆盖集从「全部当前 head revision」扩为「∪ 未裁决冲突组的 incumbent revision」，取组条件与 `_collect_typed_recall_confirmation` 的取组查询逐字一致。0.6.23–0.6.37 只覆盖当前 revision，因此冲突组的 incumbent（按定义是上一版）永远拿不到向量分，group 的向量准入只可能由 challenger 单方贡献——与 S3 §5.2「整组同进同出」不对称。**候选面不变**：`vector` lane 只对已过全部资格门的那一个 (memory_id, revision) 精确查表。**重建成本**：没有未裁决冲突组的库 manifest **一个字节不变**（每条记忆恰好一行、同序），升级不触发任何世代重建；有未裁决组时下一代多嵌入的条数 = 未裁决组数（run9 证据库 8 → 9）。组一被裁决 incumbent 立刻退出。
- **F-V-2b 修法（零 DDL）**：把混成一条的判据拆成两件事。**可用性** = 该世代按**它自己**的 `(memory_id, revision)` 重算的 manifest == 入库的 `content_hash`；`cognitive_memory_revisions` 行内不可变（触发器 `immutable cognitive revision`），故这等价于「渲染格式版本与每条 revision 的内容都没变」，不成立仍是 `cognitive_vector_stale`、整代不用。**覆盖率** = 是否覆盖当前全部可召回 revision；不覆盖也**照常可用**，只对覆盖到的 revision 打分，本次召回记新降级码 `cognitive_vector_partial`。0.6.37 之前任何一次 head 修订都让**整库**的认知向量车道 stale 到下一次世代激活为止（run9 实测 3.38 s / 15.19 s），而争议轮恰好紧随修订。自证 manifest 在装载缓存时算一次（只在世代变化/启动时跑），召回路径零额外成本；缓存装载里原来那条「ref 集合 != 当前 head 集合 → 损坏」换成「行为空 → 损坏」，行级完整性没有变弱——紧接着的 `vector_manifest_hash` 全集逐字复核仍在。`cognitive_vector_partial` 是**车道仍在服务本次召回**的降级码，世代 hash 照常落审计。
- **契约**：`slices/S3-cognitive-systems-recall.md` 新增 **§5.4-补**（租约到期是降级，六条：判据 / 理由 / 续租 / 降级说明的导出对 / 与分页时限的分野）与 **§5.3-补2**（向量对称与世代覆盖率，五条：覆盖集定义 / 重建成本 / 可用性与覆盖率分开判 / 旧行为的成因 / partial 的语义）。
- **证据库离线重放**（`native-a6-run9` 只读副本，时钟钉 `1788905502.0`）：head 清单 8 → **9**（未裁决组 `cognitive-conflict-group-cd9b2beb…` 的 incumbent r2 与 challenger r3 都在集合里）；T22 那一刻的 active 世代**自证成立**、与当前清单只差 incumbent 一条 → **`cognitive_vector_partial` 而不是 `cognitive_vector_stale`**；**全部 10 代的自证 manifest 逐代成立**（revision 行内不可变这条设计前提在真实库上验证过）；T20 更正后 3.38 s 激活的那一代在 0.6.38 下 `orphaned=0`，即争议后那 **15.19 秒**的窗口里它继续可用，9 条当前 revision 覆盖 8 条。
- **不改**：DDL 与 7.4 checksum（`cognitive_vectors` 主键本来就是 `(memory_id, revision, generation_id)`）；写路径、世代构建的三段式与 CAS、`_collect_typed_recall_candidates`、0.6.34 的向量相对阈值、entity/task_scope/temporal lane、排序权重与预算、hash 域；`page_typed_recall_result` 的 `typed_recall_result_expired` 有意保留（分页是把旧结果字节**重新取出**，不是对已下发字节的用途授权）。**降级记账**：0.6.37 读一个**有未裁决冲突组且已重建过世代**的 0.6.38 库会因「世代 ref 集合 != 当前 head 集合」抛 `active cognitive vector generation is incomplete`——这一格**不是静默退化而是开库失败**，回退需先重建一代；收据表的续租值只是一个更晚的 `expires_at`，0.6.37 读它无碍。
- **公共面**：零增减。`public-api-0.6.38.json` 除 `version` 外与 `public-api-0.6.37.json` **逐字相同**；两个新稳定码分别留在 `core.recall_context_use` 与 `features.cognitive_vector`，快照测试新增 0.6.38 段并断言它们**不在**根导出里。
- **测试**：新增 `tests/integration/test_recall_context_use_lease.py` **7 项**（①事故本身 → 收据+说明+续租长度；②续租不越过来源自己的期限；③被取代 / ④被遗忘 / ⑤policy 变化三条负控仍 fail closed；⑥租约未到期路径逐字不变；⑦epoch 前进+租约到期导出两条说明）与 `tests/integration/test_cognitive_vector_conflict_incumbent.py` **4 项**（①只有 incumbent 的向量能准入的查询确实准入；②争议窗口里无关记忆照常经向量车道召回、码是 partial；③裁决后 incumbent 退出世代；④无冲突库 manifest 逐字不变），另在 `test_typed_recall_cognitive_vector.py` 新增负控 `test_text_format_bump_still_degrades_the_whole_lane`（渲染格式版本一变仍整代 stale），合计 **12 项**。**基线 `5ad222a` 上分别有 3 项与 3 项逐条失败**，其余是控制项。本轮**有意反转的既有断言 5 处**（用途围栏 ①、confirmation 世代条数 1→2、relation 世代条数 2→3、stale→partial 两处），另加两处版本号钉。全量 `tests/` 失败集合与 main 基线（`5ad222a`，独立 detached worktree）**逐条相同**（63 项既有环境失败，`diff` 为空），1661 → **1673** passed / 8 skipped；`ruff check src tests` 与基线同为 **709**。
- **Host 侧**：**不需要配合改动**（`degradation_codes` 原样透传，无白名单）。但三件事应知道：① T18 那一类失败自己消失，事件 AA 的重采机制从「常态」退化成「兜底」，**建议保留但不必再触发**；② `RecallContext.expires_at = moment + 60` 的语义可以重新定义为「这次召回结果多久之后需要重新解释」；③ 新码 `cognitive_vector_partial` **不是故障**（出现在每次修订到下一次维护 tick 之间），看板应把它与 `cognitive_vector_stale` 分开计数——后者从此意味着「整代不可信」。仅本地候选（分支 `m0638`），未发布、未构建制品、未合 main、Host 未 pin。

## [0.6.37] - 2026-09-09（冲突组的词面准入基底扩展到 head 的 subject_entity/qualifiers；A6-8 / NC-4 的 SDK 侧阻断项）

- **缺陷来源**：Host 事件 V 备忘 `simple_harness/plans/2026-09-08-hm-to-a6/DECISION-V-CONTEST-NOTICE.md` §4.4 / §6 **F-V-2**（评审升级为 HM-TO-A6 A6-8 / NC-4 的阻断项）。HM-TO-A6 第 9 次原生旅程：T20 用户更正「统一用 Python 3.13，不是 3.12」→ head r2；T21 分析车道 `contest_semantic` → r3（`contested`）＋ 1 条未裁决的 `cognitive_conflict_groups`；T22「那你现在按哪个版本执行这套校对流程？」→ 模型答「按 Python 3.13 执行」，**没有要求确认**。Host 已修好「每一条会执行的路由都一定去问 Memory」，但**问了能不能得到答案**在 SDK 这边不成立。裁决与逐条论证见 `plans/2026-08-29-human-memory-digital-twin/DECISION-2026-09-09-contested-group-admission-basis.md`。
- **闸关住的是那串字，不是那个类**：该 group 的 `contested_slot_text` 插桩输出逐字是 `{"object_value":["Python 3.13","3.12"]}` 与 `{"predicate":"proofreading_script_python_version"}`——**一个 CJK 字符都没有**。0.6.31 起冲突组的词面准入只看这段文本，于是纯中文的用户原句永远零命中；Host 重放里能出通知，只是因为**模型的转述**逐字复述了「统一用 3.13，不是 3.12」（那是缺陷报告，不是机制）。而用户真会说出口的那几个字，恰在这条 head 自己的 `qualifiers`（`["在做资料校对时"]`）里。
- **修法**：`features/conflict_slot.py` 新增 `contested_head_context_text` 与 `contested_admission_text`（＝ `contested_slot_text` ∪ head 当前 revision 的 `subject_entity`/`qualifiers`）、`CONTESTED_HEAD_CONTEXT_FIELDS`、`CONFLICT_ADMISSION_TEXT_VERSION = 1`（槽位文本本身一个字节没改，故 `CONFLICT_SLOT_TEXT_VERSION` 仍是 1）；四个名字都**不进根导出**。`backends/sqlite_v5.py::_collect_typed_recall_confirmation` 在**两名成员都通过全部资格门之后**取 `revision == group.challenger_revision` 的**公开** payload 作 `head_payload`（`h.current_revision = g.challenger_revision` 是取组查询的前提）。不传 `head_payload` 即逐字回退到 0.6.31。
- **只对冲突组生效**：普通 item 车道的词面准入不变。0.6.31 收窄防的是「**兄弟记忆的**词面把 group 拉进来」（事件 O / F-O-3），而本轮扩展的是「**group 自己 head 的**词面」——group 只有一个 head，兄弟记忆的谓词与取值仍然永远不准入 group（用例各钉一次）。
- **代价（有意接受，写进契约）**：查询命中 head 自己的 `subject_entity`/`qualifiers` 时，按冻结的 `RecallDecisionV4` 二选一规则，该轮变成 confirmation-only，同轮普通 items 被扣住——F-O-3 抱怨的形状在**这一格**回归。裁定：**「不让模型拿一个未裁决的争议值去执行」优先于「同轮多返回几条无关记忆」**。`test_typed_recall_conflict_short_circuit.py` 的 ① 因此按新政策改写，是本轮**唯一**一处被有意反转的既有断言。
- **契约**：`slices/S3-cognitive-systems-recall.md` 新增 `§5.3-补（2026-09-09，0.6.37）`六条——§5.3 只说「contested 只能走完整 group confirmation」，从未规定 group 凭什么算相关，本轮把这条未写下的默认补成条款：基底定义；只对冲突组生效；无向量世代时同样生效；只读公开 payload、在全部资格门之后判定、§5.2 整组原子性与隐私门一字不变；已知代价。
- **证据库离线重放**（`.local-test-evidence/2026-09-09/native-a6-run9/…/human_memory_v7.db` 只读副本，剔除晚于 T22 的遗忘指令 `suppression-directive-1dba5ee…`，时钟钉 `1788905502.0`，无 embedder 故词面是唯一车道）：T22 用户原句 `recall`/items=3/**0 组** → `needs_user_confirmation`/items=0/**1 组**（逐字命中证据里的 `cognitive-conflict-group-cd9b2beb…`，incumbent r2 `Python 3.13` / challenger r3 `3.12`）；负控「今天天气不错，随便聊聊」修前修后都 **0 组**；两条正控（模型转述、谓词查询）修前修后都 **1 组**。Host 的验收信号 `contested_probe_admitted` 因此从 `model_query` 翻成 `user_turn`。
- **顺带查清的两件事**（Host 备忘要求）：**(a)** 「该 head 在 T22 没有向量世代」不准确——r2 的向量在更正后 **3.38 s** 激活（世代 `0a55e0c1…`），争议后 **15.19 s** 又建了含 r3 的 `5adb7b43…`，T22 时它已就位 **51.4 s** 且 manifest 与当前 head 清单一致。真正成立的是更强的结构事实：`_cognitive_vector_head_rows_unlocked` 只取 `r.revision = h.current_revision`，**世代永远只覆盖 head 的当前 revision**，因此**冲突组的 incumbent 成员永远拿不到向量分**。世代重建**不受召回 deadline 约束**（`_prepare_cognitive_vector_lane` 只判「有没有 active 世代 / manifest 对不对得上 / 嵌入来不来得及」，任一不成立就退化，从不构建；构建在 Host 维护 tick 上，`timeout=60 s`、退避 60–600 s）。记账：head 一变 manifest 立刻对不上，从该次修订到下一次成功激活之间（实测 3.4 s / 15.2 s），**整库**认知向量车道都是 `cognitive_vector_stale`——而争议轮恰好紧随修订。记为 F-V-2a / F-V-2b，本轮不修。**(b)** 争议短路发生在**准入之后**（`execute_typed_recall`：普通候选 → confirmation 收集 → 非空才丢弃 items）。§5.3 在**泄露方向**上从未被破坏（普通车道的 `_cognitive_recall_state_allowed` 不带 `allow_contested`，contested head 永不作为普通 item 下发）；被破坏的是条款背后的目的「依赖该值的任务必须要求确认」——head 连候选都不是时，冲突对这一轮不存在。本轮让**这个组**可达而不放宽任何披露判据：准入仍逐组判定，另一个组不会被顺带披露；§5.2 的整组原子性与隐私门仍然优先（任一成员不可见则整组连同「存在冲突」一起扣下，且 contested head **也不会**退化成普通 item）。
- **不改**：DDL 与 7.4 checksum（不加表/列/索引，已有库不需要迁移或重建世代）；写路径、向量世代构建、`_collect_typed_recall_candidates`、0.6.34 的向量相对阈值、entity/task_scope/temporal lane、排序权重与预算；**降级安全**——准入基底不落库、不进任何 manifest 与 hash 域，0.6.36 读一个 0.6.37 用过的库看到的字节完全一样。
- **公共面**：零增减。`public-api-0.6.37.json` 除 `version` 外与 `public-api-0.6.36.json` **逐字相同**；快照测试增加 0.6.37 段并断言四个新符号**不在**根导出里。
- **测试**：新增 `tests/integration/test_typed_recall_contested_group_admission.py` **8 项**，以证据形状离线复现（槽位文本无 CJK、无向量世代、未裁决的冲突组、中文查询）：① 中文用户原句 → `needs_user_confirmation`/items=0/1 组、两名成员带 exact revision；② 负控 0 组；③ 两条正控仍 1 组；④ 库里**没有任何** `cognitive_vector_generations` 行时仍准入（`degradation_codes == ('cognitive_vector_unavailable',)`）；⑤ 兄弟记忆自己的词面永不准入 group；⑥ 无冲突库上同一条中文查询照旧走普通车道；⑦ 成员证据被遗忘 → 整组扣下且不退化成普通 item；⑧ 纯函数（不传 head 逐字回退、head 上下文只含两个字段、未知类型/非 Mapping/None fail closed）。**基线 `b1f9492` 上 ① 与 ④ 逐条失败**，②③⑤⑥⑦ 在基线上即通过（它们是控制项）。全量 `tests/` 失败集合与 main 基线（`b1f9492`，独立 detached worktree）**逐条相同**（63 项既有环境失败，`diff` 为空），1653 → **1661** passed / 8 skipped。
- **Host 侧**：**不需要任何配合改动**（`contested_probe_admitted` 自动翻成 `user_turn` 就是验收信号）；但 Host 应知道 §5.3-补.6 的代价——命中争议 head 主题的那一轮，`memory_standalone` 会从「若干 fragments」变成「confirmation-only、零 fragments」。仅本地候选（分支 `m0637`），未发布、未构建制品、未合 main、Host 未 pin。

## [0.6.36] - 2026-09-09（离线车道可以提交自己召回时所用的 Procedure 适用性指纹；F-S1 的第二条闭合）

- **缺陷来源**：0.6.35 裁决备忘 §6 第 2 条 / §9 **F-S1b（P0）**，再往上是 Host 事件 S 备忘 `simple_harness/plans/2026-09-08-hm-to-a6/DECISION-S-RELATION-KEYERROR.md` §3「坑一」/ §4.4 / §7。0.6.35 修好了关系端点**解析**，但 Host 的 `check()` 走的是 `check_history_visibility`，而 `backends/history_visibility.py` 在调用 `_validate_recall_context_use_sources_unlocked` 时写死 `procedure_applicability_fingerprints=frozenset()`（注释：*never reuse old runtime fingerprints*），于是 `_cognitive_recall_type_authority_allowed_unlocked` 对任何 Procedure 返回 False → `RECALL_AUTHORITY_STALE` → `history_source_stale` → Host 整批 `analysis_candidate_no_longer_visible`。Host 因此仍然只能在下发前扣下所有 Procedure 端点，A6-6 的 Procedure 形态依旧不可达。裁决与逐条论证见 `plans/2026-08-29-human-memory-digital-twin/DECISION-2026-09-09-analysis-lane-applicability-fingerprints.md`。
- **那句注释本身是对的**：`check_history_visibility` 拿到的是 `DisclosureContext`（本轮之前 `core/port.py:337`、`core/manager.py:197`）而不是 `RecallContext`；让它顺手去读那条已存召回请求里的指纹，等于让一次**任意晚**的复核继承一次**任意早**的运行时状态——「此刻是否适用」会被一份陈旧快照冒名顶替。本轮要解决的正是这个两难：既不让前台复核 replay 旧指纹，又让一条**本来就没有活着的 Run** 的离线车道能用它召回时所用的同一份事实完成复核。
- **否决 (a)「让 `check_history_visibility` 自己去读那条召回请求的指纹」**：那正是注释禁止的行为，且对**所有**调用方生效（前台历史页、当前输入观察、短时域来源展开一起继承旧指纹），复核会退化成回放。**否决 (b)「加一个跳过 Procedure 适用性的布尔开关」**：它把「这一条为什么可以放行」变成「调用方要求不检查」，收据里只剩一个 `True`，与事件 S 备忘 §4.4 批评过的「装好引信的洞」同型。
- **采纳 (c)：显式 provenance 的 attestation + Memory 自己的观测审计佐证**。新增根导出 `ProcedureApplicabilityProvenance`（闭合枚举，今天只有 `applied_use_fingerprints` 一个成员）、`ProcedureApplicabilityAttestation`（构造期要求已排序、去重、1..256 条，同一集合恒得同一 `attestation_hash`）、`ProcedureApplicabilityReceipt`；`check_history_visibility` / `MemoryManager` facade / `CognitiveMemoryBackend` 协议同步增加**可选** `procedure_applicability` 参数。provenance 的含义被契约钉死为「这些指纹属于 Memory 已经消费过其观测的那些使用」——即**「曾经真的用过」，不承诺「此刻仍适用」**。
- **关键：这个标签不被信任**。放行一条 Procedure 来源要求三条同时成立——① 端点这一版 `procedure_records.applicability_fingerprint` 不是 `UNBOUND_PROCEDURE_APPLICABILITY`；② 它在调用方提交的集合里（既有的类型权威门）；③ **该 memory 上至少一条 `outcome='success' AND attributable=1` 的 `procedure_observations` 携带逐字相同的指纹（本轮新增）**。两条判据分工要说准：**②（既有）已经把调用方钉死在 Memory 自己存的那个值上**——`procedure_records.applicability_fingerprint` 只由观测提交路径写入，所以编出来的指纹本来就过不了 ②；**③（新增）加的是另一件事**：把「这个指纹被*绑定*过」收紧为「它被一次*成功且可归因的使用*背书过」。第一次观测无论成败都会绑定指纹，因此健康库上存在一条 ③ 真正起作用的路径：指纹由一次失败观测绑定，之后该 Procedure 由 `EXPLICIT_USER` 的 correct/supersede 从 draft 直接推到 active（合法，epoch 只在 CREATE/REVISE 重置故指纹被继承）——①② 与状态门都放行，③ 拒绝。③ 的匹配口径是 **memory + 指纹**，不含 revision 与 `qualification_epoch`（复制链上指纹逐字继承，一条旧 revision 的成功观测可以背书当前 head，这是有意的）。`procedure_observations` 是 Memory 自己的表：append-only、不可变触发器、open 时逐字复核（本轮实测：改掉其中一列，库直接开不起来，报 `procedure observation columns differ`）。诚实记账：本轮为 ③ 写的用例走的是**篡改库**那条路，上面那条健康库路径已识别但未单写用例。
- **其余各门一条不少**：head CAS、`content_hash`、状态门（Procedure 仍必须 `active`/`reinforced`）、有效时间、隐私类与信息属性、记忆与逐条证据的 suppression、整批的 `_ordinary_recall_disclosure_allowed` / 收件人绑定 / 主体一致，全部照原样先跑。attestation 只把「Procedure 恒 stale」变成「Procedure 按既有判据裁定」。
- **fail closed 与缺省不变**：不提交 ⇒ 逐字等于 0.6.35（用例断言快照 `to_json()` 里**连 `procedure_applicability` 这个键都不出现**，两个哈希因此也逐字相同）；提交给当前输入观察入口 ⇒ `MemoryValidationError('history_current_input_rejects_procedure_applicability')`；非 canonical 类型 ⇒ `TypeError`。`MemoryManager` facade **只在调用方真的提交时才转发这个 kwarg**，早于 0.6.36 的后端因此照常工作。
- **收据**：`HistoryVisibilitySnapshot` **在 `schema_version` 之后**追加 `procedure_applicability`（既有 7 参位置式构造逐字不变），内容为 provenance / `attestation_hash` / `fingerprint_count` / **逐条命中的 binding hash**（按调用方绑定顺序）；attestation 同时计入 `request_hash`。「哪几条来源是因为这份 attestation 才可见」因此可被离线复核。**确定性**：集合成员判定与 `EXISTS` 与行序/时钟/并发无关；同一时钟下同一请求两次调用 `to_json()` 与 `snapshot_hash` 逐字相同（用例断言）。
- **重新裁定 `applied_use_fingerprints`**（事件 S 备忘 §4.1 要求 F-S1 解除时必须重做）：**可以作为复核依据**。① 保住的那一半被**收紧到「成功且可归因地用过」**——「指纹必须等于 Memory 自己存的那个值」由既有判据 ② 保证（该列只由观测提交写入，Host 编不出来），判据 ③ 在此之上要求它被一条成功可归因的观测背书，于是「用过」不再包含「只是被一次失败观测绑定过」；② 佐证来源被链锚定（篡改则库不可开）；③ 它只是复核不是授权，端点在 apply 时仍被逐门重解析。**诚实记账**：一个工具后来改名/换签/被撤的 Procedure，其前台召回会消失，但离线车道仍可把它作为关系端点提交，代价是可能出现一条 `applies_to` 指向「当前不可用、但确曾被真实用过」的流程；该边仍带 exact revision，遗忘/suppression/冲突/状态各门照常生效，且「曾经用过」本就是一条历史陈述。要彻底消掉这条残余需把「适用性快照」持久化成可离线重算的事实，记为 **F-S1c**，另立。
- **契约**：`slices/S3-cognitive-systems-recall.md` 新增 `§2-补2（2026-09-09，0.6.36）`，七条——缺省即 0.6.35；唯一入口与 provenance 语义；作用面只到 `HistoryRecallBinding`；三条放行判据与「其余各门一条不少」；收据形状；**重新裁定 §2-补.6 的取舍**；F-S1 两条至此全部闭合（§2-补.7 作废）。
- **不改**：DDL（7.4 checksum 不变，不加表/列/索引，**已有库不需要迁移或重建世代**）；**降级安全**——快照的新键只在返回值里、不落库，0.6.34/0.6.35 读一个 0.6.36 用过的库看到的字节完全一样；写路径（观测提交 / 信号提交 / mutation apply）一个字节未动；前台 `execute_typed_recall` 及其三处 `context.procedure_applicability_fingerprints` 未动；`cognitive_relations` 与 twin graph 投影、收据视图、分类决定链复核均未改。**代价（记账，不修）**：判据 ③ 每条 Procedure 绑定多两次点查——`procedure_records` 主键命中，加上 `procedure_observations` 上一次**索引搜索**（查询里的 `outcome='success' AND attributable=1` 恰好蕴含部分索引 `procedure_observation_success_scope_unique` 的谓词，实测 `EXPLAIN QUERY PLAN` 为 `SEARCH … USING INDEX procedure_observation_success_scope_unique (memory_id=?)`，非覆盖但不是扫描）。不加任何索引，DDL 与 7.4 checksum 不动；一批复核里 Procedure 绑定数以关系端点候选数为上界（≤ 8）。
- **公共面**：`public-api-0.6.36.json` 是 0.6.19 以来**第一次**真正的根导出扩张——只多上述三个名字，`removed_public_methods` 与 `migrations` 逐字未变，没有任何名字被拿走；快照测试改为显式断言这一点，并断言 facade 上 `procedure_applicability` 存在且默认 `None`。快照只记名字，记不到签名：`CognitiveMemoryBackend` 协议（`core/port.py`）同样多了这个可选形参，第三方后端**运行时**照常工作（该协议没有 `runtime_checkable`，全仓无 `isinstance` 检查，facade 也只在调用方真的提交时才转发），但静态类型检查会要求它跟进。
- **测试**：新增 `tests/integration/test_history_procedure_applicability.py` **9 项**（Procedure 走满三次真实观测进 `active`，用真实指纹做公开 typed recall 再绑成 `HistoryRecallBinding`，与 Host 分析车道逐字同形）——缺省仍 stale 且快照无新键；带 attestation 即可见且收据四项齐全、两次调用逐字相同、`request_hash` 与不带的那次不同；指纹对不上不放行；**判据 ③ 单独量出**（篡改三条观测的指纹 → 同一 attestation 变 stale，随后断言这样的库重开即 fail closed）；同批 semantic 绑定不受影响也不进命中清单；当前输入入口拒绝 + 非 canonical 类型 `TypeError`；`generation=STALE` 时仍不可见（不绕过任何既有门）；attestation canonical 形状 fail closed；facade 只在提交时转发 kwarg。基线 `b82187e` 上整文件收集即失败（公共面新增，预期）。快照测试增加 0.6.36 段；`test_memory_0623_schema_cutover.py:77` 版本钉死顺延到 0.6.36（7.4 checksum 不变）。全量 `tests/` 失败集合与 main 基线（`b82187e`，独立 detached worktree）**逐条相同**（63 项既有环境失败，`diff` 为空），1644 → **1653** passed / 8 skipped；`ruff check src tests` 计数与基线同为 709。
- **Host 侧**：本轮同步解除扣留（分支 `worktree-f-s1b`）。Host 按**能力探测**（根导出三个类型 + facade 有 `procedure_applicability` 参数）而不是版本号决定是否解除 `sdk_procedure_endpoint_unresolvable`，因此同一份 Host 代码在 0.6.34/0.6.35 上继续扣留、在 0.6.36 上放行；召回时的指纹被持久进候选快照，`check()` 用**同一份**构造 attestation。**F-S1 两条至此全部闭合**，A6-6 的 Procedure 形态端到端可达（Host 侧实测：`cognitive_relations` 1 行 `applies_to` 指向观测提交出来的那一版 head，孪生图出现对应的边）。仅本地候选（分支 `m0636`），未发布、未构建制品、未合 main、Host 未 pin。

## [0.6.35] - 2026-09-09（关系端点的分类决定由血缘上最近的已分类祖先承担；观测提交出来的 Procedure head 不再被判损坏）

- **缺陷来源**：Host 事件 S 备忘 `simple_harness/plans/2026-09-08-hm-to-a6/DECISION-S-RELATION-KEYERROR.md` §3「坑二」与 §8 后继 **F-S1（P0，SDK）**。分析协议 v8 允许 `semantic_relation` 用 `ExistingMemoryTarget` 指向已有记忆；Procedure 端点必须先走满三次独立成功观测进 `active` 才可能被召回、才可能被下发，而 Host 实测那条 Procedure 的 head 是 **revision 4、分类行只有 revision 1**，`_resolve_semantic_relation_payload_unlocked` 因此抛 `MemoryCorruptionError('relation endpoint classification is missing')`，**整批分析死掉**（同批 episode 一起丢）。Host 只能在下发前按名扣下所有 Procedure 端点（`sdk_procedure_endpoint_unresolvable`），验收 A6-6 的 Procedure 形态在 0.6.34 上不可达。裁决与逐条论证见 `plans/2026-08-29-human-memory-digital-twin/DECISION-2026-09-09-procedure-relation-endpoint-classification.md`。
- **根因**：`cognitive_memory_revisions` 全仓只有两个写入点——mutation apply（`sqlite_v5.py:8528`，每个 operation 必配一条分类决定）与 `_copy_cognitive_revision_unlocked`（`:11392`，Procedure 观测提交 / Prospective 信号提交）。后者是**逐字复制**：`content_json` / `content_hash` / `effective_privacy_class` / `information_attributes_json` / 有效时间 / 证据 span / task-scope origin 全部照抄，只改 `lifecycle_state` 与 plan/operation 标识，**从不重跑分类策略**——被分类的三样东西一个字节都没变，本来也不需要重跑。所以「每条 revision 都必须有自己的分类行」**从来不是本仓的不变量**；0.6.34 的端点解析把它当成了不变量，于是把一条完全健康的血缘判成了损坏。同一条缺陷对 Prospective 端点同样成立（同一个复制函数），只是至今下发的 Prospective 端点都还停在 revision 1。
- **否决方案 (a)「观测提交时补写一条分类行」**，三条理由任一独立成立：① **伪造审计**——该表的语义是「分类策略在某个 plan 的某个 operation 上作出的裁定」，行里钉着 policy 四元组、逐条证据权威、闭合键集合的 `decision_json` 与 `decision_hash UNIQUE`，且被 `_decode_and_verify_mutation_receipt_row_unlocked`（`:9730` 起）逐字重算；观测提交并没有跑过分类策略，补写的行只能是编出来的；② **救不了存量库**——只对将来的提交生效，A6-6 仍要等库里的 Procedure 重走三次观测，覆盖存量必须迁移/重建；③ 会改写 twin 血缘 refs 的 `classification` 一支（`:15522`）。
- **采纳 (b)：管辖某一版的分类决定 = 血缘上最近的已分类祖先**。端点解析改查 `memory_revision <= 端点 revision` 的最大已分类 revision（`MAX` 子查询 + `JOIN cognitive_memory_revisions`），三条判据：① 恰好一行且 `decision_hash` 非空，否则**语句逐字不变**的 `'relation endpoint classification is missing'`——一条已分类祖先都没有仍然是真损坏；② **管辖决定必须仍然逐字描述这一版**——决定行的 `effective_privacy_class` / `effective_attributes_json`（被 `decision_hash` 锚定、收据复核逐字重算）必须等于端点 revision 行的对应两列，**exact revision 与继承祖先两条路径都查**，否则新增码 `'relation endpoint classification differs'`；③ 继承路径再要求祖先与端点两版 `content_hash` 相等（复制函数逐字搬运内容，一旦分叉就不再由那条决定管辖），否则新增码 `'relation endpoint classification lineage differs'`。判据 ② 与 ③ **0.6.34 完全没有**——它连「这一行是不是仍然描述这一版」都不问。诚实记账：被接受的端点 revision 集合**确实变大了**（这正是修复目的），同时新增了两道从前没有的校验；判据 ③ 比较两条 revision 行的 `content_hash`，是必要条件而非「机器证明」，已绕过不可变触发器并把 `content_json`/`content_hash` 改成自洽一对的对手仍可构造（本轮为该形状补了用例），但隐私类/属性那一路变体在收据复核 `_decode_and_verify_mutation_receipt_row_unlocked`（`:10166-10183`，该处**从不查分类表**，本身即「每版一行」从来不是不变量的独立旁证）处仍会被抓住。确定性：`MAX(memory_revision) <= 端点 revision` 是全序上的唯一解；「至多一条决定指向同一 revision」由结构保证（决定行的 `memory_revision` 恒取自同事务刚插入的 `revision`，而 `cognitive_memory_revisions` 主键是 `(memory_id, revision)`），与行序、时钟、并发无关。
- **契约**：`slices/S3-cognitive-systems-recall.md` 新增 `§2-补（2026-09-09，0.6.35）`——Task 2「任何 classification authority 缺失整批拒绝」的适用范围是 mutation **operation** 而非每条 revision；祖先归属规则与两条继承自证条件；**关系端点解析的完整门序**（此前从未入契约）；端点隐私类/属性作为关系记忆分类的下界；F-S2 的离线车道口径；F-S1b 未闭合。
- **不改**：DDL（7.4 checksum 不变，**已有库不需要迁移，也不需要重建任何世代**；且**降级安全**——本轮不写新行不加新列，0.6.34 读 0.6.35 写过的库看到的字节完全一样）；写路径 `record_procedure_observation` / `apply_prospective_signal` / `_copy_cognitive_revision_unlocked` / `_copy_procedure_payload_unlocked` 一个字节未动，**不补写任何分类行**（用例显式断言修复后分类行集合仍是 `(1,)`）；端点解析其余各门与其顺序（所有权 / head CAS / `conflict_status` / `_cognitive_recall_state_allowed`——Procedure 仍必须 `active`/`reinforced` / 有效时间 / canonical content 与 `content_hash` 复算 / typed payload 行 / 证据 span 非空 / `restricted` 不可披露 / 记忆与逐条证据 suppression）；`cognitive_relations` 与 twin graph 投影、mutation 收据视图、分类决定链复核。根导出零增减；快照 `public-api-0.6.35.json` 除 `version` 外与 0.6.19 起逐字相同。
- **测试**：新增 `tests/integration/test_procedure_relation_endpoint.py` **6 项**——① 三次真实 `record_procedure_observation` 把 Procedure 推到 `active`（head=revision 4、分类行仅 `(1,)`，即事故现场），关系 plan 提交成功，`cognitive_relations` **恰好 1 行**且 `applies_to` 指向 `procedure@4`，twin graph **1 条边**且**两端**都在节点集合里，reopen 后逐字相同，分类行集合仍是 `(1,)`；② typed recall（`FULL_TEXT` + 真实 `applicability_fingerprint`）命中的 exact revision 与**落库的那条边**指向同一版（读 `cognitive_relations` 回来比对，而非与同一个 Python 变量比）；③ 整条分类链摘掉后仍 `classification is missing`，并断言这样的库**连重开都不该成功**（收据链复核先一步 fail closed）；④ 关掉→篡改端点 revision 的 `information_attributes_json`→重开 → `endpoint classification differs`；⑤ 关掉→把 `content_json` 与 `content_hash` 改成自洽的一对→重开 → `classification lineage differs`；⑥ **Prospective 同形**：`REGISTRATION_ACCEPTED` + `TIME_DUE` 到 `TRIGGERED`（head=revision 2、分类行仍只有 `(1,)`）走同一条继承路径并落边——`apply_prospective_signal` 与观测提交共用同一个复制函数，这条路 0.6.35 起首次可解析。除守卫用例 ③ 外 5 项在基线 `36dac46` 上红、本分支绿（把文件原样拷到基线 detached worktree 实跑核对）。快照测试增加 0.6.35 段；`test_memory_0623_schema_cutover.py:77` 版本钉死顺延到 0.6.35（7.4 checksum 不变）。全量 `tests/` 失败集合与 main 基线（`36dac46`，独立 detached worktree）**逐条相同**（63 项既有环境失败，`diff` 为空），1638 → **1644** passed / 8 skipped；`ruff check src tests` 计数与基线同为 709。
- **独立评审**（opus，只读）：1 条 MUST-FIX（新不变量与端点门序未入 S3 契约，且 `S3:51-52` 那句「任何 classification authority 缺失整批拒绝」未被正面回应）**已改**；8 条 NIT 全部处置——把继承校验改成比对**被哈希锚定的决定行**而非两条未锚定的 revision 行并覆盖 exact 路径、两个篡改用例改回全仓惯用的 close→tamper→reopen（第三个不可能这么写，改为显式断言「这样的库重开即 fail closed」）、typed recall 用例改为真正观测到修复、twin graph 断言补上 source 端、`§4` 小标题与「更严」口径改写、`§8` 数字内联、新增 Prospective 用例、索引缺失的两次扫描代价记账（不加索引，避免动 DDL）。
- **Host 侧**：**F-S1 只解除了一半，扣留暂不能撤**。端点解析这一半已修；`check_history_visibility` 对 Procedure 永远 stale 那一半未动——`backends/history_visibility.py` 调用 `_validate_recall_context_use_sources_unlocked` 时写死 `procedure_applicability_fingerprints=frozenset()`，`_cognitive_recall_type_authority_allowed_unlocked` 对任何 Procedure 返回 False → `RECALL_AUTHORITY_STALE` → `history_source_stale`。那不是 bug 修复而是一次**公共 API 扩面**（`core/port.py` 抽象方法 + `core/manager.py` facade + 后端实现同时改），且会把备忘 §4.1 那笔取舍（`applied_use_fingerprints` 保住「用过」、放弃「此刻仍适用」）从"今天没有暴露面"变成"直接决定 `check()` 放不放行"——备忘自己写明该取舍在 F-S1 解除时**必须重新裁定**，本轮不替 Host 作这个决定，记为 **F-S1b** 另立版本。因此 Host 现在解除 `sdk_procedure_endpoint_unresolvable` 只会把整批打在 `check()` 那一关（备忘 §4.4）。仅本地候选（分支 `m0635`），未发布、未构建制品、未合 main、Host 未 pin。

## [0.6.34] - 2026-09-09（认知向量准入阈值：同型相对判据 + 绝对护栏；0.45 单一常数对中文短记忆没有余量）

- **缺陷来源**：Host 语料重跑复核 `simple_harness/plans/2026-09-07-corpus-c01-local/RUN-RERUN-FLASH-01-REVIEW.md` §五缺陷 3。同一条中文短 semantic 记忆、同一个语义、两次都算合理的改写，与目标记忆的余弦分别是 **0.4354** 与 **0.5839**——**跨在冻结阈值 `COGNITIVE_VECTOR_MIN_SCORE = 0.45` 两侧**。C01-19 / C06-06 / C06-08 三例 FAIL 同根：向量世代已激活、查询与记忆语义等价但**词面零重叠**，结果 `fragments=[]`，模型据此宣告"没存过"。即召回可靠性依赖模型措辞抖动。裁决与逐条论证见 `plans/2026-08-29-human-memory-digital-twin/DECISION-2026-09-09-vector-score-margin.md`。
- **测量**（可复现，见备忘 §1）：用生产同一嵌入器（Host `deskpet/memory/wemm_embedder.py` → `tencent/WeMM-Embedding-2B` 本地快照，`encode_document(normalize_embeddings=True)`）离线复算；记忆向量**直接取自证据库** `cognitive_vectors`（不重新嵌入，与线上判定所用向量逐字节相同），77 例 / 142 条已激活向量 / 79 次 `context_route` typed recall 查询。**目标对**（同例、命中该次请求类型）n=117：min 0.0601 / p05 0.3048 / p10 0.4109 / p25 0.4865 / 中位 0.5738 / p75 0.6624 / max 0.8337；**无关对**（跨例、同请求类型）n=7 562：min −0.0779 / 中位 0.2818 / p90 0.4007 / p95 0.4363 / p99 0.5067 / max 0.6251。C01-19 那一对离线复现为 0.4343 / 0.5822（与 Host 相差 0.001–0.002，设备精度差）。
- **两个候选方案被实测证伪**：**(a) 只下调常数不可行**——同义改写下界 0.0601 **低于**无关上界 0.6251，两条分布完全重叠，"下有余量、上在无关之上"的可行域为空；硬压到 0.35 会把无关准入从 3.74% 抬到 21.85%（约 6 倍）。**(c) 让 Host 词面通道兜底不可行**——三例的 `typed_recall_query_terms` 与存储 payload 交集**全为 0**，且 C06-06（`图表说明 / 要配文字说明`）与 C06-08（`方案说明顺序 / 先结论`）的 predicate **本来就是中文**，评审表推测的"英文 predicate 稀释"只解释得了 C01-19 一例；另两例是纯粹的中文同义鸿沟。
- **采纳 (b)：阈值改为「同一记忆类型内的相对判据 + 绝对护栏」**。`features/cognitive_vector.py` 新增两个冻结参数 `COGNITIVE_VECTOR_RELATIVE_FLOOR = 0.35`、`COGNITIVE_VECTOR_RELATIVE_RATIO = 0.90` 与两个纯函数 `cognitive_vector_effective_min_score` / `cognitive_vector_admits`：`effective(type) = min(0.45, max(0.35, 0.90 × 该类型内已过全部资格门的最高余弦))`，准入 ⟺ `cosine ≥ effective`，等价于「`≥ 0.45` 或（`≥ 0.35` 且 `≥ 0.90 ×` 同类型最高分）」。`COGNITIVE_VECTOR_MIN_SCORE` **仍是 0.45、一个字节没动**，只从"唯一判据"变成"准入上界"。
  - **`RATIO = 0.90` 只救援"该类型内与最佳并列"的候选**——落后最高分 10% 以上的一律不进；这是"救援"而不是"把阈值调低"。0.85 / 0.80 的召回收益（+0.8 / +1.7pp）与其无关准入涨幅（+0.31 / +0.99pp）和候选膨胀不成比例。
  - **`FLOOR = 0.35` 是"整库都不相关时不救援"的绝对护栏**（C07 零召回子集的唯一保护）：比无关分布中位 0.2818 高 0.067，同时是**能覆盖 C06-06 目标 0.3690 的最大整值**（0.38 就救不回，0.30 落在无关 p75 之下、形同虚设）。
  - **相对参照必须按记忆类型分组**（本次最关键的结构选择）：C06-06 正是 procedure/episode 侧 0.6017 与 semantic 侧 0.3690 同处一次召回的形状，若参照全局最高分，semantic 的下限会被推到 0.5415、目标必然落空。与既有实现同构（lane 排序与预算本就 `for memory_type in requested_types` 分组），也直接服务于"多提类型 ≤ 15%"。
- **实现**：`backends/sqlite_v5.py` 三处判定点改走新规则。普通候选收集 `_collect_typed_recall_candidates` 因此**分两趟**——第一趟只过资格门并暂存四条既有 lane 的原始分与余弦（`lane_ranks` 留空），第二趟算出逐类型最高分后统一判定并用 `dataclasses.replace` 补齐 `lane_ranks`；`lane_ranks` 的排列次序（`full_text` 之后、`entity` 之前）与 0.6.33 逐字一致。争议 group（0.6.31 槽位级准入）同样把成员判定推迟到两名成员过完资格门之后，相对参照 = 两名成员与**同类型普通候选**的最高余弦；group 级的「成员必须不低于同类型任一普通候选」判据一字不变。
- **效果**：并库扫描目标召回 **83.8% → 87.2%**，无关准入 **3.74% → 4.01%**（+0.27pp），每次召回准入候选数 4.82 → 5.13。三个 FAIL 用例按真实候选池（该例记忆 ∩ 该次请求类型）离线重算**全部转为准入**：C01-19 semantic 0.4343（effective 0.3909）、C06-06 semantic 0.3690（effective 0.35，procedure 0.6017 照旧准入）、C06-08 semantic 0.4039（effective 0.3635）。
- **pin**：`effective ≤ 0.45` 恒成立，故 **0.6.33 下已经进入 `vector` lane 的候选在 0.6.34 下全部仍然进入**——本次只可能放宽、不可能收紧，判定不变的召回其 `decision_hash` / `result_hash` / `receipt_hash` 逐字不变（单测以全域扫描与 121×121 网格逐点等价钉死）。
- **审计**：阈值动态化后必须可审计——`typed_recall_terminals.terminal_json.cognitive_vector` 增加 `admission` 块（`min_score` / `relative_floor` / `relative_ratio` + 逐请求类型的 `effective_min_score`）。该块**只在 `cognitive_vector` 已存在时出现**，即只有真的跑了向量通道的召回其终态 hash 会变，未请求 vector 的召回终态逐字不变。记录原始余弦有先例（短时域审计的 `vector_lane[].score`）。
- **不改**：DDL（7.4 checksum 不变，**已有库不需要重建世代**）、世代身份 / manifest / `COGNITIVE_TEXT_FORMAT_VERSION` / 向量文本渲染、资格门顺序与四条既有 lane 计分、RRF 权重与 `lane_cap` 与预算裁剪、四个退化码与 `DECISION-2026-09-07` §4.4 的「冷态只退化不失败」、`typed_recall_query_terms`。根导出零增减（新参数与新函数只经 `features.cognitive_vector` 可达）；快照 `public-api-0.6.34.json` 除 `version` 外与 0.6.19 起逐字相同。
- **测试**：新增 `tests/unit/test_vector_score_margin.py` 8 项（参数取值；`effective ≤ 0.45` 全域扫描；≥ 0.45 恒准入＝pin 的机器证明；`effective` 形式与 OR 形式在 121×121 网格上逐点等价；最高分低于护栏时全域不准入；最高分自身越过护栏必准入；`None` 分不入 lane；0.4354 / 0.5839 这一对的落点）与 `tests/integration/test_typed_recall_vector_score_margin.py` 5 项（用**合成向量精确复现**那一对分数：同一世代只换查询，两次改写都召回同一条记忆且审计里的生效下限分别是 0.39186 与 0.45；整库最高分 0.30 时零召回且生效下限落在 0.35；同类型内落后 10% 以上的候选不被救援；episode 0.6017 不压制 semantic 0.3690 且审计逐类型分开；未请求 vector 的终态不含新键）。快照测试增加 0.6.34 段；`test_memory_0623_schema_cutover.py:77` 版本钉死顺延到 0.6.34（7.4 checksum 不变）。全量 `tests/` 失败集合与 main 基线（`c070119`，独立 detached worktree）逐条相同（63 项既有环境失败，`diff` 为空），1625 → 1638 passed / 8 skipped；`ruff check src tests` 计数与基线同为 709。`ARCHITECTURE/*.md` 三份文档里 0.6.23 的阈值理由（「同义改写通常 ≥0.6」）已被本轮实测证伪，同步改写。
- **Host 侧**：**不需要改任何调用**。三条后继：① **C07 零召回子集必须回归**——本轮 rerun 证据里 `no_recall=true` 与 `privacy_allowed=false` 的用例数为 **0**，护栏 0.35 只有分布证据、没有端到端证据；② C06-06 类的语义鸿沟仍在（0.3690 离护栏只有 0.019 余量），若要真正稳住，方向是给 semantic 的向量文本补一段确定性中文渲染（对标 0.6.26 给 prospective 加的 `cognitive_text_supplement`），那会改 `COGNITIVE_TEXT_FORMAT_VERSION` 并整代重建，另行立项；③ 语料回执建议读 `terminal_json.cognitive_vector.admission.effective_min_score`，把"向量该中没中"的争议从推测变成读数。仅本地候选（分支 `m0634`），未发布、未构建制品、未合 main、Host 未 pin。

## [0.6.33] - 2026-09-09（候选收集段耗尽预算时不再抛 `OperationalError`：取消安全的开事务原语）

- **缺陷来源**：Host 401 矩阵第四轮 `simple_harness/plans/2026-09-07-corpus-c01-local/TYPED-RECALL-401-RUN-10.md` §4.4 附带发现 / §10 后继第 1 条（"偶发候选侧缺陷"，Host 12 次采样命中 5 次）。`deadline_ms=1`（契约允许的最小合法预算）耗尽在**候选收集阶段**时，抛 `sqlite3.OperationalError: cannot start a transaction within a transaction` 而不是契约要求的 `TimeoutError("DEADLINE_EXCEEDED")`，且那一次不落 deadline 终态行。裁决与逐条论证见 `plans/2026-08-29-human-memory-digital-twin/DECISION-2026-09-09-deadline-in-collection.md`。
- **根因**：`aiosqlite` 的后台线程（`aiosqlite/core.py:57-74` 的 `_connection_worker_thread`）拿到排队项之后**无条件**执行语句，只在 future 已完成时把结果丢掉——**取消 `await` 并不取消已经排队的语句**。而 0.6.32 及以前每一处事务都写成 `await self._db.execute("BEGIN …")` 在 `try:` **之外**、`finally` 里才回滚；typed recall 的候选收集段（`backends/sqlite_v5.py` 三处 `asyncio.wait_for`）恰恰靠取消兑现 deadline，于是取消落在 `_resolve_suppression_unlocked`（`:1710`）那条读快照 `BEGIN` 上时，语句照样执行、`finally` 一次都不跑，连接上留下一个**孤儿事务**；紧接着 `_persist_typed_recall_timeout`（`:6632`）的第一条语句 `BEGIN IMMEDIATE` 就抛 `OperationalError`，终态写不下去。**比 Host 观察到的更严重**：孤儿事务留在连接上，之后每一次写入（含下一次召回的 admit）都继续抛同一个错——一次超时污染整条连接。对称的第二个缺口：`COMMIT` 被取消之后 COMMIT 仍会执行，此时四处裸写的 `except BaseException: await execute("ROLLBACK")` 抛 `cannot rollback - no transaction is active`，把正在展开的 `CancelledError` 整个替换掉。
- **修法**：新增内部原语 `backends/sqlite_tx.py`（`backends.*` 从来不在公共面上）。`begin_transaction(db, statement="BEGIN IMMEDIATE")` 把 BEGIN 单独排队并 `asyncio.shield`；取消时**先等它落地拿到确定结论**，真的开了事务就在同一条串行队列上补一条 ROLLBACK，然后才把取消继续往上抛，因此任何取消路径都不可能留下开着的事务。`rollback_transaction(db)` 保证回滚永远不替换它正在收尾的异常。召回路径（含它经过的 `history_source_guard` / `history_visibility` / `procedure_discovery` / `prospective_sources` / `source_admission` / `operation_audit` 六个 helper）**全部 47 处** `BEGIN*` 改走新原语，**语句文本逐字不变**；四处裸写的 `except BaseException: ROLLBACK` 改走 `rollback_transaction`，其余 44 处 ROLLBACK 本就安全、逐字未动。`_persist_typed_recall_timeout` 另加一条纵深防御（拿到写锁后若 `in_transaction` 为真先回滚），让「终态必然落库」不依赖任何上游路径的正确性。**不在范围内**：`backends/sqlite.py`（v4 旧后端、另一个连接对象、不在 typed-recall 路径上）与 `migrations/*`、`backends/upgrade_validation.py`（同步 `sqlite3`，没有取消）。
- **阶段命名**：入账**之后**的每一处预算耗尽改抛 0.6.27 已有的 `TypedRecallDeadlineExceeded(stage)`（仍是 `TimeoutError` 子类，`str(exc)` 仍是 `DEADLINE_EXCEEDED`，Host 到 `context_route_recall_timeout` 的映射一字不变），`stage` 取 `after_admission` / `history_source_context` / `cognitive_vector_lane` / `collect_write_lock` / `collect_candidates` / `after_collect_candidates` / `collect_confirmation` / `collect_short_candidates` / `authority_expired`；`admit_write_lock` 仍是唯一一个**没有**终态行的阶段（幂等记录尚未落库）。`stage` **只活在异常对象上、不进任何持久行**——终态行的形状与入账阶段到期时逐字相同（`("deadline_exceeded", NULL, NULL, 0, 0, "[]", "[]")`）。
- **修复后的三条义务**：任何阶段耗尽预算都 (1) 抛稳定的 `TimeoutError("DEADLINE_EXCEEDED")`、(2) 恰好落下**一条**与入账阶段同形状的 deadline 终态行、(3) 连接上不留任何事务，随后同 key 同 body 的重放拿回同一个终态、其他 key 的召回照常成功。
- **不改**：DDL（7.4 checksum 不变）、根导出零增减、公共错误类、冻结枚举与 Harness v4 wire 形状、资格门 / lane 排序 / 预算 / RRF、终态与收据的任何 hash 域、写锁纪律。快照 `public-api-0.6.33.json` 除 `version` 外与 0.6.19 起逐字相同。
- **测试**：新增 `tests/integration/test_typed_recall_deadline_in_collection.py` 3 项——① 原语层**成对**见证（同一时序下裸写法留下孤儿事务、下一条 `BEGIN IMMEDIATE` 抛 `cannot start a transaction within a transaction`；`begin_transaction` 之后 `in_transaction is False` 且下一条 BEGIN 正常）；② **确定性**构造：把候选收集段那条读快照 `BEGIN` 的 `await` 端拖过 deadline（BEGIN 已在连接上落地），逐条断言上述三条义务 + `stage == "collect_candidates"`；③ 50 次 `deadline_ms=1` soak：只允许 `DEADLINE_EXCEEDED`（任何别的异常直接判回归）、每次之后连接上无事务、**每个已入账请求恰好一条**终态行、最后一次正常预算召回成功。负控：后两项拷到 0.6.32 基线 worktree 跑，两项都以 `OperationalError: cannot start a transaction within a transaction` 失败。快照测试增加 0.6.33 段；`test_memory_0623_schema_cutover.py:77` 版本钉死顺延到 0.6.33（7.4 checksum 不变）。全量 `tests/` 失败集合与 main 基线（`eec9760`，独立 detached worktree）逐条相同（63 项既有环境失败，`diff` 为空），1622 → 1625 passed / 8 skipped；`ruff check src tests` 计数与基线同为 709。
- **Host 侧**：**不需要改任何调用**。401 矩阵 RUN-10 §10 第 1 条可以从"偶发缺陷、建议单独立格"改判为已修；`budget:deadline` 的判据不必再刻意把预算耗在入口/入账阶段——收集段到期现在同样稳定地给出 `DEADLINE_EXCEEDED` + 恰好一条终态行 + 可重放。仅本地候选（分支 `m0633`），未发布、未构建制品、未合 main、Host 未 pin。
