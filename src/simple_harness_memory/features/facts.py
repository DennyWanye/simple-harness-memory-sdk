...
class RuleBasedFactExtractor(FactExtractor):
    _RULES: tuple[tuple[str, str, re.Pattern[str], float, int], ...] = (
        ...
        (
            "profile",
            "occupation",
            re.compile(r"我(?:是|做|干)(?:一名|一个|一位|名|个|位)?([^，。！？,、\s的]{1,20})"),
            0.6,
            1,
        ),
        (
            "profile",
            "location",
            re.compile(r"我(?:住在|在|来自)([^，。！？,、\s的]{1,20})"),
            0.6,
            1,
        ),
        (
            "profile",
            "language",
            re.compile(r"我(?:常用|使用|说)的语言?是?([^，。！？,、\s的]{1,20})"),
            0.5,
            1,
        ),
        (
            ...
            "pet_name",
            re.compile(
                r"(?:我)?(?:养了?|有一只)(?:一只|一条|一个|只|条|个)?(?:叫|名字叫)?([^，。！？,、\s的]{1,20})(?:的)?(?:狗|猫|宠物|仓鼠|鱼)"
            ),
            0.7,
            1,
        ),
        (
            ...
            "prefers",
            re.compile(
                r"(?:很|非常|比较|特别)?(?:喜欢|爱|偏好|偏爱)(?:吃|喝|看|听|用)?([^，。！？,、\s的]{1,30})"
            ),
            0.55,
            1,
        ),
        ...
        (
            "goal",
            "goal",
            re.compile(r"我(?:打算|计划|目标(?:是)?|希望)([^，。！？,、\s的]{1,50})"),
            0.6,
            1,
        ),
        (
            ...
            "event",
            re.compile(
                r"(?:昨天|今天|前天|上周|上个月|最近)(?:我)?(?:去了|到了|在)([^，。！？,、\s的]{1,20})"
            ),
            0.55,
            -1,
        ),
        ...
        try:
            resp = await self._client.chat.completions.create(
                ...
                messages=[
                    {
                        "role": "user",
                        "content": "提取事实为 JSON 数组(key/category/value/confidence)：\n"
                        + content,
                    }
                ],
                temperature=0,
