...
FAMILIES = (
    ...
    'typed_terminal', 'recall_context_use', 'short_recall', 'suppression', 'job_transition',
)
...
class OperationAuditItemV1:
    ...
    def __post_init__(self) -> None:
        if self.family not in FAMILIES or self.outcome not in ('committed', 'rejected', 'started', 'observed', 'unknown'):
            raise MemoryValidationError('operation_audit_item_scope_invalid')
        _identifier(self.event_kind)
        if len(self.event_kind) > 64 or any(c not in 'abcdefghijklmnopqrstuvwxyz0123456789_' for c in self.event_kind):
        ...
        if type(self.occurred_at) not in (int, float) or not math.isfinite(self.occurred_at) or self.occurred_at < 0:
            raise MemoryValidationError('operation_audit_time_invalid')
        object.__setattr__(self, 'occurred_at', float(self.occurred_at))
        object.__setattr__(self, 'item_hash', _hash('memory.operation.audit.item.v1', self.to_json()))

    def to_json(self) -> dict[str, Any]:
        return {name: list(value) if isinstance(value, tuple) else value
                for name in self.__dataclass_fields__ if name != 'item_hash'
...
class OperationAuditExpectation:
    ...
    def to_json(self) -> dict[str, str]:
        return dict(family=self.family, event_ref_hash=self.event_ref_hash, receipt_hash=self.receipt_hash)
...
class OperationAuditCoverage:
    ...
    def __post_init__(self) -> None:
        ...
        for name in ('supported_event_kinds', 'exclusions', 'unresolved_ref_hashes', 'missing_event_ref_hashes'):
...
class OperationAuditPage:
    ...
    def __post_init__(self) -> None:
        ...
        for name in ('items', 'coverage', 'expectation_results'):
            object.__setattr__(self, name, tuple(getattr(self, name)))
        object.__setattr__(self, 'page_hash', _hash('memory.operation.audit.page.v1', self.to_json()))
    ...
    def to_json(self) -> dict[str, Any]:
        ...
        return dict(schema_version=1, coverage_version=self.coverage_version,
                    principal_ref_hash=self.principal_ref_hash, snapshot_hash=self.snapshot_hash,
                    items=[v.to_json() for v in self.items], coverage=[v.to_json() for v in self.coverage],
                    ...
                    enumeration_complete=self.enumeration_complete, all_operations_recorded=False)
