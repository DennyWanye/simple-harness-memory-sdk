...
class ConversationMemoryRole(str, Enum):
    USER = "user"
    ASSISTANT = "assistant"


class ConversationMemoryQueryStatus(str, Enum):
    COMPLETE = "complete"
    TRUNCATED = "truncated"
    TIMEOUT = "timeout"


class ConversationMemoryApplyStatus(str, Enum):
    APPLIED = "applied"
...
def canonical_message_payload_hash(
    *, source_event_id: str, user_id: str, session_id: str,
    role: str | ConversationMemoryRole, memory_text: str | None,
) -> str:
    role_value = ConversationMemoryRole(role).value
    canonical_text = (
        None if memory_text is None else canonicalize_memory_text(memory_text)
    )
    payload = {
...
def canonical_recall_query_hash(
    *, user_id: str, session_id: str, query_text: str,
...
class ConversationMemoryRecallQuery:
    ...
    @classmethod
    def create(
        cls, *, context_query_id: str, user_id: str, session_id: str,
        query_text: str, max_items: int, max_bytes: int,
        timeout_seconds: float,
...
class ConversationMemoryAdapter:
    ...
    async def apply(self, intent: _IntentLike) -> ConversationMemoryApplyResult:
        ...
        try:
            role = ConversationMemoryRole(getattr(intent.role, "value", intent.role))
        ...
        except (MemoryOwnershipConflict, MemoryValidationError, ValueError) as exc:
