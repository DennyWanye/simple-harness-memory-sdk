"""Per-call safe metadata, following OA1's Host-persisted observation boundary."""
...
class ProspectiveSourceReadObservationV1:
    ...
    reason: str
    observed_at: float
    operation: str = "read_prospective_outbox_source"
    ...
    def __post_init__(self):
        ...
        if (type(self.schema_version) is not int or self.schema_version != 1
                or self.operation != "read_prospective_outbox_source"
        ...
        object.__setattr__(self, "observation_hash",
            _hash("memory.prospective.source.observation.v1", self.to_json()))
...
async def observed_read(manager, *, principal, outbox_id, payload_hash):
    invocation = _hash("memory.prospective.source.invocation.v1", str(uuid4()))
    request = (_hash("memory.prospective.source.request.v1", [outbox_id, payload_hash])
    ...
    def observe(outcome, reason, source_hash=None):
        ...
        observation = ProspectiveSourceReadObservationV1(
    ...
    try:
        result = await manager._backend.read_prospective_outbox_source(
