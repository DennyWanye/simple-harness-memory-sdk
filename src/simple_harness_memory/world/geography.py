...
class StaticWeatherProvider(WeatherProvider):
    ...
    async def fetch(self, location):
        if self._weather is None:
            return None
        return Weather(location=location, temperature_c=self._weather.temperature_c, description=self._weather.description, humidity=self._weather.humidity, fetched_at=time.time())
    ...
    async def fetch(self, location):
        ...
        try:
            async with httpx.AsyncClient(timeout=10) as client:
                resp = await client.get(f"{self._base_url}/weather", params={"q": location, "appid": self._api_key, "units": "metric"})
