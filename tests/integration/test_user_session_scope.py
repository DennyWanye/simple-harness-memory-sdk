...
async def test_user_scoped_reads_and_immutable_session_binding(tmp_path):
    ...
    first = await backend.append_message(
        "session-a", "user", "共享词 青松",
        user_id="user-a", source_event_id="event-a",
    )
    await backend.append_message(
        "session-b", "user", "共享词 白桦",
        user_id="user-b", source_event_id="event-b",
    )
    assert all(
        "白桦" not in hit.text
        for hit in await backend.recall("共享词", user_id="user-a")
    )
    assert await backend.get_message(
        first.message_id, user_id="user-b"
    ) is None
    with pytest.raises(MemoryOwnershipConflict):
        await backend.append_message(
            "session-a", "user", "steal",
            user_id="user-b", source_event_id="event-steal",
        )
