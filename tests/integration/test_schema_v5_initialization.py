...
@pytest.mark.asyncio
async def test_fresh_v5_schema_is_atomic_idempotent_and_reopens_same_receipt(
    tmp_path: Path,
) -> None:
    ...
    receipt = receipts[0]
    assert receipt.schema_version == SCHEMA_VERSION == 5
...
async def test_every_v5_initialization_fault_reopens_to_one_valid_receipt(
    tmp_path: Path,
    fault_point: str,
) -> None:
...
@pytest.mark.asyncio
async def test_tampered_v5_is_rejected_read_only_before_wal_or_chmod(tmp_path: Path) -> None:
    path = tmp_path / "tampered-v5.db"
    backend = SQLiteHumanMemoryBackend(path)
    await backend.initialize()
    await backend.close()
    connection = sqlite3.connect(path)
