...
async def test_sqlite_facts_and_recall(tmp_path):
    backend = SQLiteMemoryBackend(str(tmp_path / "mem.db"), auto_extract_facts=True)
    await backend.initialize()
    await _append(
        backend,
        "s1",
...
async def test_sqlite_decay_forgets_old_event(tmp_path):
    ...
    assert await backend.get_facts(category="event", active_only=True, user_id=USER) == []
    await backend.close()
...
async def test_extract_facts_does_not_log_key_value(tmp_path, capsys):
    backend = SQLiteMemoryBackend(str(tmp_path / "mem.db"), auto_extract_facts=True)
    await backend.initialize()
    await _append(backend, "s1", "user", "我养了一只叫Max的狗", "sqlite-4")
...
async def test_append_atomic_rollback(tmp_path, monkeypatch):
    ...
    with pytest.raises(RuntimeError):
        await _append(backend, "s1", "user", "我养了一只猫", "sqlite-rollback")
...
async def test_delete_session_cascades(tmp_path):
    backend = SQLiteMemoryBackend(str(tmp_path / "mem.db"), auto_extract_facts=True)
    await backend.initialize()
    await _append(backend, "s1", "user", "我养了一只叫Max的狗", "sqlite-5")
    await _append(backend, "s2", "user", "今天天气很好", "sqlite-6")
    assert await backend.delete_session("s1", user_id=USER) == 1
    assert await backend.get_recent_messages("s1", user_id=USER) == []
    assert await backend.get_recent_messages("s2", user_id=USER)
    assert all(fact.value != "Max" for fact in await backend.get_facts(user_id=USER))
    await backend.close()
...
async def test_resource_limits_raise(tmp_path):
    ...
    with pytest.raises(MemoryLimitError):
        await _append(facts, "s1", "user", "我养了一只叫Max的狗", "limit-3")
    await facts.close()
...
async def test_delete_session_rebuilds_twin(tmp_path):
    backend = SQLiteMemoryBackend(str(tmp_path / "mem.db"), auto_extract_facts=True)
    await backend.initialize()
    await _append(backend, "s1", "user", "我养了一只叫Max的狗", "sqlite-9")
    assert "Max" in (await backend.get_digital_twin(user_id=USER)).relationships.entities
    await backend.delete_session("s1", user_id=USER)
    assert "Max" not in (await backend.get_digital_twin(user_id=USER)).relationships.entities
    await backend.close()
...
async def test_delete_old_sessions_is_user_scoped_and_bounded(tmp_path):
    ...
    await backend._conn.execute(
        "UPDATE sessions SET last_activity_at = ? WHERE user_id = ? AND session_id = 'old'",
        (time.time() - 100 * 86400, USER),
    )
    await _append(backend, "recent", "user", "recent msg", "sqlite-11")
    deleted = await backend.delete_old_sessions(older_than_days=30, user_id=USER, limit=1)
    assert deleted == 1
