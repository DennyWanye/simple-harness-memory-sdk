...
async def capture_expected(db):
    ...
    specs = (
        ...
        ('mutation_rejection', 'memory_mutation_rejection_audits', 'rejection_id', 'rejection_hash', ''),
        ...
        ('short_recall', 'short_horizon_audit', 'audit_id', 'audit_hash',
    ...
    for family, table, key, digest, where in specs:
        async with db.execute(f'SELECT {key},{digest} FROM {table} {where} ORDER BY rowid') as cursor:
            result.extend((family, m.operation_audit_ref_hash(family, row[0]), row[1])
...
async def mixed(tmp_path):
    from tests.integration.test_cognitive_mutation_repository_v5 import (
        _admitted, _Authority, _classification_policy, _operation, _plan, _span,
    )
    from tests.integration.test_short_horizon_repository_v5 import (
        _Authority as ConversationAuthority, _registration, NOW,
    )
    ...
    manager = await m.build_human_memory_v7(
        tmp_path / 'mixed.db', clock=lambda: NOW, evidence_authority=evidence,
    ...
    try:
        ...
        with pytest.raises(m.MemoryValidationError, match='classification_policy_required'):
            await manager.apply_memory_mutation_plan(
                principal=p, scope=m.MemoryScope.personal(p.actor_id), plan=_plan(envelope, _operation(span)),
            )
    finally:
        await manager.close()
    manager = await m.build_human_memory_v7(
        tmp_path / 'mixed.db', clock=lambda: NOW, evidence_authority=evidence,
        audit_access_authority=audit, classification_policy=_classification_policy(),
        conversation_evidence_authority=ConversationAuthority(tuple(x[0] for x in pairs)),
    )
    try:
        result = await manager.apply_memory_mutation_plan(
            principal=p, scope=m.MemoryScope.personal(p.actor_id),
            plan=_plan(envelope, _operation(span), plan_id='actual-commit', idempotency_key='actual-commit'),
        )
        assert result.outcome is h.MemoryMutationApplyOutcome.COMMITTED
        no_plan = replace(_plan(envelope, _operation(span), base_revision=2,
                                plan_id='actual-no-mutation', idempotency_key='actual-no-mutation'),
                          outcome=h.MemoryMutationPlanOutcome.NO_MUTATION, operations=())
        await manager.apply_memory_mutation_plan(principal=p, scope=m.MemoryScope.personal(p.actor_id), plan=no_plan)
        context = _context(expires_at=NOW + 100)
        execution = await manager.execute_typed_recall(
            principal=p, context=context, plan=_recall_plan(context, idempotency_key='actual-recall'),
        )
        assert execution.result.items
        item = execution.result.items[0]
        fragment = h.ContextFragmentBindingV2('fragment', 'f' * 64)
        await manager.authorize_recall_context_use(principal=p, request=h.RecallContextUseAuthorizationRequestV1(
            p.actor_id, context.run_id, context.turn_id, 'actual-provider-attempt',
        ...
        for registration, reference in pairs:
            await manager.ingest_committed_evidence(registration.envelope, registration.admission_receipt)
            await manager.register_conversation_evidence(reference)
        await manager.rebuild_short_horizon_projection(principal=p)
        short = await manager.recall_short_horizon(principal=p, query='Project alpha', disclosure_context=context.disclosure_context)
        assert short.hits
        directive = await _suppress(manager, p)
        await manager.revoke_suppression(principal=p, request=m.SuppressionRevokeRequest(
            'revoke-actual', p.actor_id, directive.directive_id, 'user_restore', NOW,
        ))
...
async def test_real_mixed_all_nonjob_families_receipts_and_pages(tmp_path):
    ...
    try:
        ...
        assert {f for f, _, _ in expected} == {
            'mutation_commit','mutation_rejection','typed_request','typed_attempt',
            'typed_terminal','recall_context_use','short_recall','suppression',
        }
...
@pytest.mark.parametrize('limit_name,reason', [
...
async def test_real_budget_exhaustion_no_partial_page_and_cleanup(tmp_path, monkeypatch, limit_name, reason):
