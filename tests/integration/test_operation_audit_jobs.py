...
from tests.integration.test_memory_061_core import (
    _build_pipeline, _evidence, _HostEvidenceAuthority, _HostExecutor, _ingest, WORKER_CONFIG,
)
...
async def setup(tmp_path, *, run=True, clock=None):
    ...
    manager, runner = await _build_pipeline(
        tmp_path/'memory.db', executor, evidence_authority=authority,
        audit_access_authority=audit, now=lambda: now[0],
    )
    try:
...
@pytest.mark.parametrize('kind', ['provider_handoff', 'result_committed', 'application_staged', 'mutation_audit_committed'])
...
async def test_recomputed_event_hash_cannot_rebind_result_to_foreign_batch(tmp_path):
    ...
    try:
        ...
        await db.execute('UPDATE job_attempt_events SET result_hash=?,event_hash=? WHERE event_id=?',
...
async def test_actual_retry_and_reclaim_keep_old_cursor_and_actual_attempts(tmp_path):
    ...
    try:
        ...
        assert await manager.backend.fail_analysis_batch(first,'actual_provider_failure',WORKER_CONFIG) is m.WorkerRunOutcome.RETRY_SCHEDULED
        clock[0] += 20
        ...
        await manager.close()
        manager = await m.build_human_memory_v7(tmp_path/'memory.db', clock=lambda:clock[0], audit_access_authority=audit)
        ...
        assert [i.event_kind for i in events] == ['provider_handoff','reclaimed','retry_scheduled','provider_handoff']
