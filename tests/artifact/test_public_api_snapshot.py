...
def test_current_candidate_sources_and_docs_are_consistent() -> None:
    ...
    assert "当前 source candidate：**0.6.21**" in readme
...
def test_public_api_0_6_21_keeps_0_6_19_surface() -> None:
    previous = json.loads(Path(__file__).with_name("public-api-0.6.19.json").read_text())
    for version in ("0.6.20", "0.6.21"):
    ...
    assert simple_harness_memory.__version__ == "0.6.21"
