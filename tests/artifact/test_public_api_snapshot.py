...
def test_public_api_0_5_2_snapshot_is_frozen_and_0_5_1_is_preserved() -> None:
    snapshot_path = Path(__file__).with_name("public-api-0.5.2.json")
    snapshot = json.loads(snapshot_path.read_text(encoding="utf-8"))
    previous = json.loads(
        Path(__file__).with_name("public-api-0.5.1.json").read_text(encoding="utf-8")
    )
    assert snapshot["package"] == "simple-harness-memory-sdk"
    assert snapshot["version"] == simple_harness_memory.__version__ == "0.5.2"
...
def test_0_5_2_candidate_sources_and_docs_are_consistent() -> None:
    ...
    assert pyproject["project"]["optional-dependencies"]["harness"] == [
        "simple-harness-sdk>=0.4,<0.7"
    ]
    assert "simple-harness-sdk>=0.4,<0.7" in pyproject["project"]["dependencies"]
    readme = (ROOT / "README.md").read_text(encoding="utf-8")
    changelog = (ROOT / "CHANGELOG.md").read_text(encoding="utf-8")
    assert "当前 source candidate：**0.5.2**" in readme
    assert "已发布 fallback 为 0.5.1" in readme
    assert "## [0.5.2] - 2026-08-25" in changelog
