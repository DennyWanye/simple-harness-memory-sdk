...
def test_current_candidate_sources_and_docs_are_consistent() -> None:
    ...
    assert "当前 source candidate：**0.6.20**" in readme
...
def test_public_api_0_6_20_keeps_0_6_19_surface_with_cjk_typed_recall_fix() -> None:
    previous = json.loads(Path(__file__).with_name("public-api-0.6.19.json").read_text())
    snapshot = json.loads(Path(__file__).with_name("public-api-0.6.20.json").read_text())
    assert previous["version"] == "0.6.19"
    assert {k: v for k, v in previous.items() if k != "version"} == {
        k: v for k, v in snapshot.items() if k != "version"
    }
    assert simple_harness_memory.__version__ == snapshot["version"] == "0.6.20"
