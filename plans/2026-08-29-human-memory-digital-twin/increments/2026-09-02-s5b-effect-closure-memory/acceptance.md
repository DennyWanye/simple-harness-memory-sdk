...
## A16 增量 AC：analysis 输出预算必须容得下真实提案（2026-09-04）

**背景**：S5b 终验实测暴露，`max_output_tokens=2048`（`memory_ingestion_outbox.py:340`）
对真实提案而言余量不足。8 轮实测分布：

| output_tokens | 457 | 908 | 1111 | 1381 | 1561 | 1939 | **2048** | **2048** |
|---|---|---|---|---|---|---|---|---|
| 记忆物化 | 2 | 2 | 2 | 3 | 3 | 3 | **0** | **0** |

成功轮上界 1939，距上限仅剩 109 token；两次失败轮**恰好顶格 2048**，即被截断。

**为什么不是「让截断可重试」（依据经独立评审订正）**：

> 初稿给的两条依据都不成立，特此订正。① 截断返回的是**合法** `no_mutation`，
> `_derive_envelope` 不抛异常，因此根本走不到 `analysis_derivation_failed` 的重放路径；
> ② 「零重复 Provider 调用」是最小验证动作对成功路径的**观测项**，不是禁止二次调用的
> 规范条款——实测计 PASS 的 `20260904T143009` 就有两次 Provider 调用（一次协议错 + 一次成功）。

**正确依据**：规范性约束是「每 committed turn group ≤1 次 post-turn analysis 调用」
（本文件非功能边界）与 AC-2③ 的幂等要求。截断重投会顶破该调用上限，且截断是**可预防**的
（预算够就不会发生），预防优于事后补救。**已知缺口**：若提案真的超过新预算，目前仍无补救，
记入遗留清单。

**预算不能单独调（P0，独立评审 F-01）**：同批 8 轮数据线性拟合 `latency ≈ 24.1ms/token`，
60s 的 deadline 只够约 2500 token。只提预算不动 deadline，新预算上半段物理上到不了，
而一旦触及就从「succeeded + no_mutation（可观测、可审计）」翻转成
`sent_unknown → 拒绝再投递 → dead_letter`，**比它要修的截断更糟**。
实测 `20260904T141720` 在 2048 token 已耗 55.9s（deadline 的 93%）。
故 `deadline_ms` 同步 60s→180s，并由一致性用例钉死「预算 × 实测 ms/token <= deadline」。

**新增 AC（挂 S5B-AC-2 之下，次要）**：
- ① `max_output_tokens` 必须为实测成功上界留出足够余量；取 **6144**（= 1939 的 3 倍余，
  按「最大成功值的 3 倍」定，不是凑一个刚好能过的数）。
- ② 预算不足时**不得静默收敛成成功**：`closure_reason=analysis_response_unusable`
  + 日志告警 + 审计事件（A16 之前已由 P0-16 落地，此处纳入 AC 固化）。
- ③ `deadline_ms` 必须能真的承载该预算（见上）；两常量由一致性用例绑定，不得各改各的。
- ④ oracle：用例必须断言**生产装配出来的 config**，不是函数默认值——评审 F-05 实测
  前一版只读 `inspect.signature(...).default`，把生产调用点写死 2048 后照样全绿。
  截断分支与模型主动放弃分支的理由码不得合并；executor 的告警+审计分支同样要有守护。

**验证口径**：S5B-S1 重跑，`output_tokens` 不再出现顶格值；成功率按真实分布如实披露，
不挑轮次。
