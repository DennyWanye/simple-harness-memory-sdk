...
**为什么不是「让截断可重试」**：现有重试路径 `analysis_derivation_failed:*` 明确是
「从持久响应重放，retry 永不调用 Provider」（`analysis_executor.py:440-442`）——重放同一个
被截断的响应只会重复失败直到 dead_letter。而重新调用 Provider **违反本 acceptance 已批准的
非功能约束「零重复 Provider 调用」**。因此正解是**别让它截断**，不是事后补救。
...
- ③ oracle：常量有决定性用例守护「显著大于实测成功上界」；截断分支与模型主动放弃分支
  的理由码不得合并。
