...
- Real protocol int5 and string'5' produce distinct existing reasons; SQLtrace + backend call trap
  assert no invocation of backend for either. Observer receives exactly the actual frozen rejection
  projection; original exception/rejection_receipt is unchanged. Callback error and cancellation
  preserve honest delivery status, no extra SQL, no recursive notification.
...
semantics; exact job-event required matrix; observer timeout/cancellation behavior; finite family/root
